...
top_customers_tbl %>% 
    ...
    ggplot(aes(revenue, bikeshop_name), color = ) +
...
pct_sales_by_customer_tbl <- bike_orderlines_tbl %>% 
    ...
    group_by(bikeshop_name) %>% # Using group_by to be able to calculate the proportion for each group
...
pct_sales_by_customer_tbl %>% 
    ...
    labs(
        ...
        y = "Customer",
        caption = str_glue("Customer that prefer Road: 
