...
RColorBrewer::brewer.pal(n = 100, name = "Blues"[9])
...
sales_by_year_category_2_tbl %>% 
    ...
    ggplot(aes(year, revenue, color = category_2)) + # globally
    geom_line(size = 1) + # locally
    geom_point(color = "dodgerblue", size = 5) # locally
...
sales_by_year_category_2_tbl %>% 
    ...
    geom_col(position = position_dodge(width = .9), color = "white")
...
sales_by_year_category_2_tbl %>% 
    ...
    labs(
        ...
        y = "Revenue ($M)",
        fill = "2nd Category",
        caption = "Bike sales trend look strong heading into 2016"
    ) +
    
    # Theme
    theme_light() +
    theme(
        title = element_text(face = "bold", color = "#0830GB")
    )
