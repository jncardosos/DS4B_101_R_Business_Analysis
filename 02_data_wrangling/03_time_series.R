...
I / dminutes(1) # interval / minutes = how many minutes in the interval
...
bike_sales_y_tbl %>% 
    ...
    mutate(pct_dif_1_chr = scales::percent((pct_diff_1)))
...
bike_sales_m_tbl %>% 
    group_by(year) %>% # Key Concept: Grouped mutates (Vector Operations on grouped data) are applied to each group idependently. This is very useful
...
## A rolling mean is a simple way to expose the trend in a time series. By taking an average of several values, we can reduce the effect of outliers enabling the analyst to visualize a trend.

bike_sales_m_tbl %>% 
    
    mutate(rol_mean_3 = rollmean(sales, k = 3, na.pad = TRUE, align = "right", fill = NA)) %>% 
    
    mutate(rol_mean_6 = rollmean(sales, k = 6, na.pad = TRUE, align = "right", fill = NA))
