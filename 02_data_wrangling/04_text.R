...
str_c("Oder Line: ", order_id, ".", order_line,
      " sent to Customer", bikeshop_name,
      sep = "")

str_glue("Oder Line: {order_id}.{order_line} sent to Customer: {bikeshop_name}")
...
bike_orderlines_tbl %>% 
    ...
    mutate(purchase_statement = str_glue(
        "Oder Line: {order_id}.{order_line} sent to Customer: {str_to_upper(bikeshop_name)}"
    ) %>% as.character())
...
c("CAAD12", "CAAD", "CAAD8") %>% str_replace_all(pattern = "[:digit:]", replacement = "")

# Tibble

bikes_tbl %>% 
  select(model) %>% 
  mutate(model_num_removed = model %>% str_replace_all("[:digit:]", "") %>% str_trim()) 
...
bikes_tbl %>% 
  ...
  # separete using sapaces
  separate(col    = model, into = str_c("model_", 1:7), 
           sep    = " ", 
           remove = FALSE, 
           fill   = "right", 
           extra  = "drop") %>% 
