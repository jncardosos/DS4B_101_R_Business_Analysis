...
bike_features_tbl <- bike_orderlines_tbl %>% 
    ...
    separate_bike_model(keep_model_column = 1, append = 1)
...
model_01_linear_lm_sample <- linear_reg(mode = "regression") %>% 
    set_engine("lm") %>% 
    fit(price ~ category_2 + frame_material, data = train_tbl)

model_01_linear_lm_sample %>% 
...
model_01_linear_lm_sample$fit %>% class()

model_01_linear_lm_sample$fit %>% 
    broom::tidy() %>% 
    arrange(p.value) %>% 
    mutate(term = term %>% as_factor() %>% fct_rev()) %>% 
...
model_01_linear_lm_sample %>% calc_metrics(train_tbl)
...
model_02_linear_la_complex <- linear_reg("regression") %>% 
    set_engine("lm") %>% 
    fit(price ~ .,data = train_tbl %>% select(-id, -model, -model_tier))

model_02_linear_la_complex %>% calc_metrics(new_data = test_tbl)
# 3.2.2 Feature importance ----

model_02_linear_la_complex$fit %>% 
    broom::tidy() %>% 
    arrange(p.value) %>% 
    mutate(term = term %>% as_factor() %>% fct_rev()) %>% 
...
model_03_linear_glmnet$fit %>%
    ...
    scale_x_continuous(labels = scales::dollar_format()) +
    labs(title = "Linear Regression: Feature Importance",
         subtitle = "Model 03: Complex glm Model")
...
model_04_tree_decision_tree %>% calc_metrics(new_data = test_tbl)
...
model_05_rand_forest_ranger$fit %>% 
    ...
    labs(title = "ranger Variable Importance",
         subtitle = "Model 05: Ranger Random Forest")
