...
labs(
    ...
    caption = "Conclusion: Based on the Skree Plot, we select 4 clusters to segment the customer base"
)
...
kmeans_4_tbl <- kmeans_4_obj %>% augment(customer_product_tbl) %>% 
    select(bikeshop_name, .cluster)

umap_kmeans_4_results_tbl <- umap_results_tbl %>% 
    left_join(kmeans_4_tbl)
...
cluster_trends_tbl <- customer_trends_tbl %>% 
    ...
    # Join Cluster Ssigment by Bikeshop name
...
# Cluster 1 - Medium/High Road Model Preference
cluster_trends_tbl %>% 
    filter(.cluster == 1) %>% 
...
# Cluster - 2 - Low/Medium Price, Mountain Model Preference

get_cluster_trends(cluster = 2) %>% view()


# Cluster - 3 - Low/Medium Price, Road Model Preference

get_cluster_trends(cluster = 3) %>% view()

# Cluster - 4 - High End Price, Model Preference Mountain

get_cluster_trends(cluster = 4)


# Upodate Visualization

cluster_label_tbl <- tibble(
    .cluster = 1:4,
    .cluster_label = c(
        "Medium/High Road Model Preference cluster_trends_tbl %>% ",
        "Low/Medium Price, Mountain Model Preference",
        "Low/Medium Price, Road Model Preference",
        "High End Price, Model Preference Mountain"
