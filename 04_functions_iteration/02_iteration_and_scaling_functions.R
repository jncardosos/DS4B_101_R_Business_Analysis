...
# Step 1: Create a fuctnion that can be mapped to one element
...
sales_by_month_m_cross_country_tbl <- rolling_avg_3_tbl %>% 
...
sales_by_month_m_cross_country_tbl %>% 
...
fit_loess_cross_country <- sales_by_month_m_cross_country_tbl %>%  # as the loess is not a tidy function, you have to pass the data in the end of the argument
...
tidy_loess <- function(data, span = 0.2) {
    ...
    fit_loess <- loess(formula = total_price ~ month_end_num, data = data_formatted, span = span)
