...
class_detect <- function(x) {
    ...
    if (is.numeric(x)) {
        ...
        stop("In class_detect(): Value is logical!!! Should be numeric. Definitly cannot be accepted", call. = FALSE)
    } else {
...
file_header_text <- str_glue(
    "
# SEPARATE BIKE MODELS AND DETECT OUTILERS ----
    
# separate_bike_models(): A tidy function to separate the model column into engineered features
